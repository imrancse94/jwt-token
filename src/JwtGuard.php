...
class JwtGuard implements Guard
{
    ...
    public function refreshToken(): array
    {
        try{
            ...
            if(!$auth_id = $this->jwtManager->validateRefreshToken($this->request->refresh_token)){
                throw new \Error('Refresh token is invalid');
            }

            $user = $this->provider->retrieveById($auth_id);
            $this->setUser($user);

            return array_merge($this->getToken(),['user'=>$user]);
